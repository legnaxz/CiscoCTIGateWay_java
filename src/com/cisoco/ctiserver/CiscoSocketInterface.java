...
public class CiscoSocketInterface implements Runnable {
	private static final Logger logger = LogManager.getLogger( CiscoSocketInterface.class );
	...
	public int CiscoCtiOpenReq( String host, int port ) throws IOException, InterruptedException 
	{
		...
		open_req.MessageLength = ByteBuffer.allocate(4).putInt( nltoh(msg_length)).array();
		open_req.MessageType = ByteBuffer.allocate(4).putInt( nltoh(stdtypes.MT_OPEN_REQ)).array();
